...
from common.researchstudio_ideation import build_strategy_context, run_ideation_chain  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument(
        "--ideate",
        action="store_true",
        help="run the gated Idea chain after building the compact context",
    )
    ...
    if args.ideate:
        if args.stage != "idea":
            parser.error("--ideate is only valid with --stage idea")
        context = run_ideation_chain(profile, context)
    else:
